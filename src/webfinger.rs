...
impl WebFingerClient {
        ...
        if !resource.starts_with("acct:") && 
           ...
           !resource.starts_with("https:") {
            return Err(WebFingerError::InvalidResource(format!(
                "Resource '{}' must begin with 'acct:', 'http:', or 'https:'", resource
            )));
        }
    ...
    fn extract_host(&self, resource: &str) -> Result<String> {
        if resource.starts_with("acct:") {
            ...
            if parts.len() != 2 {
                return Err(WebFingerError::HostExtractionFailed(
                    format!("Invalid acct URI format: {}", resource)
                ));
            }
            
            // For acct:user@example.com, return example.com
            // Handle potential additional parts like acct:user@example.com:port/path
            let domain = parts[1].split(|c| c == ':' || c == '/').next()
                .ok_or_else(|| WebFingerError::HostExtractionFailed(
                    format!("Could not extract domain from acct URI: {}", resource)
                ))?;
            ...
            match url.host_str() {
                ...
                None => Err(WebFingerError::HostExtractionFailed(
                    format!("Cannot extract host from URI: {}", resource)
                )),
            }
        } else {
            Err(WebFingerError::InvalidResource(
                format!("Unsupported URI scheme: {}", resource)
            ))
        }
    }
}
...
mod tests {
    ...
    async fn test_extract_host_from_http() {
        ...
        assert_eq!(
            client.extract_host("http://example.org:8080/path?query=value").unwrap(),
            "example.org"
        );
    ...
    async fn test_webfinger_request() {
        ...
        let m = server.mock("GET", "/.well-known/webfinger")
            .match_query(mockito::Matcher::AllOf(vec![
                mockito::Matcher::UrlEncoded("resource".into(), "acct:user@example.com".into()),
            ]))
            .with_status(200)
        ...
        // Construct the WebFinger URL
        let mut webfinger_url = Url::parse(&format!("http://{}/.well-known/webfinger", host)).unwrap();
        ...
        assert_eq!(jrd.aliases.clone().unwrap(), vec!["https://example.com/users/user"]);
        assert_eq!(jrd.links.as_ref().unwrap().len(), 2);
        
        let profile_link = jrd.find_link("http://webfinger.net/rel/profile-page").unwrap();
        assert_eq!(profile_link.href.as_ref().unwrap(), "https://example.com/users/user");
