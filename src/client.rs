...
pub const ACTIVITY_STREAMS_JSON_LD: &str = "application/ld+json; profile=\"https://www.w3.org/ns/activitystreams\"";
...
impl ActivityPubClient {
    ...
    pub async fn fetch_actor(&self, actor_id: &Url) -> Result<Object> {
        ...
        match entity {
            ...
            _ => Err(ClientError::MissingField(format!("Expected actor object, but got a different entity type"))),
        }
    }
    ...
    pub async fn fetch_collection(&self, collection_url: &Url) -> Result<Collection> {
        ...
        match entity {
            ...
            _ => Err(ClientError::MissingField(format!("Expected collection, but got a different entity type"))),
        }
    }

    /// Fetch actor's inbox
    pub async fn fetch_inbox(&self, actor: &Object) -> Result<Collection> {
        let inbox_url = actor.additional_properties
            .get("inbox")
            .and_then(|v| v.as_str())
    ...
    pub async fn fetch_outbox(&self, actor: &Object) -> Result<Collection> {
        let outbox_url = actor.additional_properties
            .get("outbox")
            .and_then(|v| v.as_str())
    ...
    pub async fn post_to_outbox(&self, outbox_url: &Url, activity: &Activity) -> Result<Activity> {
        ...
        match entity {
            ...
            _ => Err(ClientError::MissingField("Expected activity in response".into())),
        }
    }
...
mod tests {
    ...
    async fn test_fetch_actor() {
        ...
        let m = server.mock("GET", "/users/test")
            .with_status(200)
        ...
        assert!(actor.additional_properties.contains_key("preferredUsername"));
    ...
    async fn test_post_to_outbox() {
        ...
        let m = server.mock("POST", "/users/test/outbox")
            .with_status(200)
            .with_header("content-type", "application/activity+json")
            .match_body(mockito::Matcher::Json(serde_json::from_str(request_activity).unwrap()))
