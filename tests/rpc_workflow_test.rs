...
fn test_complete_list_domains_rpc_workflow() {
    ...
    assert!(matches!(server_request.request_type, DomainRpcRequestType::ListDomains));
...
fn test_complete_get_domain_rpc_workflow() {
    ...
    if let DomainRpcResult::DomainDetails { domain } = client_response.result {
        ...
        assert!(domain_info.allowed_file_types.as_ref().unwrap().contains(&"video/mp4".to_string()));
    } else {
...
fn test_rpc_error_workflow() {
    ...
    let client_request = DomainRpcRequest::get_domain(request_id.clone(), "nonexistent.example".to_string());
...
fn test_domain_not_found_workflow() {
    ...
    let client_request = DomainRpcRequest::get_domain(request_id.clone(), "missing.example".to_string());
...
fn test_correlation_id_handling() {
    ...
    if let (MessageEnum::DomainRpcRequest(req_1), MessageEnum::DomainRpcRequest(req_2)) = (msg_1, msg_2) {
        ...
        if let (MessageEnum::DomainRpcResponse(client_resp_1), MessageEnum::DomainRpcResponse(client_resp_2)) = (client_msg_1, client_msg_2) {
            ...
            assert!(matches!(client_resp_1.result, DomainRpcResult::DomainList { .. }));
            assert!(matches!(client_resp_2.result, DomainRpcResult::DomainDetails { .. }));
        } else {
